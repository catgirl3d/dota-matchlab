-- A custom range is interpreted as inclusive UTC calendar days.
drop function if exists public.get_archive_showcase_page(bigint, text, text, text, text, text, smallint, bigint, bigint, integer);
drop function if exists public.get_archive_showcase_overview(bigint, text, text, text, text, text, smallint);
drop function if exists public.get_match_archive_page(uuid, text, text, text, text, text, smallint, bigint, bigint, integer);
drop function if exists public.get_match_archive_overview(uuid, text, text, text, text, text, smallint);
drop function if exists archive_private.archive_page(uuid, text, text, text, text, text, smallint, bigint, bigint, integer);
drop function if exists archive_private.archive_overview(uuid, text, text, text, text, text, smallint);

create function archive_private.archive_overview(
...
declare
  result jsonb;
begin
  if p_period not in ('all', '30d', '90d', 'year', 'custom')
    or p_mode not in ('all', 'ranked', 'turbo', 'all-pick')
    or p_result not in ('all', 'wins', 'losses')
    or p_party not in ('all', 'solo', 'party')
    or p_position not in ('all', 'carry', 'mid', 'offlane', 'support', 'hard-support') then
    raise exception 'Invalid archive filter';
  end if;

  if (p_period = 'custom' and (p_start_date is null or p_end_date is null or p_start_date > p_end_date))
    or (p_period <> 'custom' and (p_start_date is not null or p_end_date is not null)) then
    raise exception 'Invalid archive date range';
  end if;
  ...
  with base as (
    ...
    where (
      p_period = 'all'
      or (
        p_period in ('30d', '90d', 'year')
        and start_time >= extract(epoch from now() - case p_period when '30d' then interval '30 days' when '90d' then interval '90 days' when 'year' then interval '365 days' end)::bigint
      )
      or (
        p_period = 'custom'
        and start_time >= extract(epoch from p_start_date::timestamp at time zone 'UTC')::bigint
        and start_time < extract(epoch from (p_end_date + 1)::timestamp at time zone 'UTC')::bigint
      )
    )
...
create function archive_private.archive_page(
  p_tracked_account_id uuid,
  p_period text default 'all',
  p_mode text default 'all',
  p_result text default 'all',
...
begin
  ...
  with filtered as (
    ...
    from public.tracked_account_matches linked
    join public.tracked_accounts account on account.id = linked.tracked_account_id
    join public.dota_matches match on match.match_id = linked.match_id
    left join public.player_match_stats stats on stats.match_id = linked.match_id and stats.account_id = account.dota_account_id
    where linked.tracked_account_id = p_tracked_account_id
      and (
        p_period = 'all'
        or (
          p_period in ('30d', '90d', 'year')
          and match.start_time >= extract(epoch from now() - case p_period when '30d' then interval '30 days' when '90d' then interval '90 days' when 'year' then interval '365 days' end)::bigint
        )
        or (
          p_period = 'custom'
          and match.start_time >= extract(epoch from p_start_date::timestamp at time zone 'UTC')::bigint
          and match.start_time < extract(epoch from (p_end_date + 1)::timestamp at time zone 'UTC')::bigint
        )
      )
...
create function public.get_match_archive_overview(
  p_tracked_account_id uuid,
  p_period text default 'all',
  p_mode text default 'all',
  p_result text default 'all',
...
begin
  if exists (
    ...
    where id = p_tracked_account_id
      and user_id = ((select auth.jwt()) ->> 'sub')
  ) then
    result := archive_private.archive_overview(p_tracked_account_id, p_period, p_mode, p_result, p_party, p_position, p_hero_id, p_start_date, p_end_date);
  ...
  return archive_private.archive_overview(null, p_period, p_mode, p_result, p_party, p_position, p_hero_id, p_start_date, p_end_date);
end;
$function$;

create function public.get_match_archive_page(
  p_tracked_account_id uuid,
  p_period text default 'all',
  p_mode text default 'all',
  p_result text default 'all',
...
begin
  if exists (
    ...
    return archive_private.archive_page(p_tracked_account_id, p_period, p_mode, p_result, p_party, p_position, p_hero_id, p_cursor_start_time, p_cursor_match_id, p_limit, p_start_date, p_end_date);
  end if;
  return archive_private.archive_page(null, p_period, p_mode, p_result, p_party, p_position, p_hero_id, p_cursor_start_time, p_cursor_match_id, p_limit, p_start_date, p_end_date);
end;
$function$;

create function public.get_archive_showcase_overview(
...
declare
  ...
  overview jsonb;
begin
  select registry.tracked_account_id, account.dota_account_id, account.persona_name, account.avatar_url, account.rank_tier, account.profile_refreshed_at
  into v_showcase
  from public.archive_showcases as registry
  join public.tracked_accounts as account
    on (account.id, account.dota_account_id) = (registry.tracked_account_id, registry.dota_account_id)
  ...
  overview := archive_private.archive_overview(v_showcase.tracked_account_id, p_period, p_mode, p_result, p_party, p_position, p_hero_id, p_start_date, p_end_date);
...
create function public.get_archive_showcase_page(
  p_dota_account_id bigint,
  p_period text default 'all',
  p_mode text default 'all',
  p_result text default 'all',
...
as $function$
declare
  v_tracked_account_id uuid;
begin
  select tracked_account_id into v_tracked_account_id
  from public.archive_showcases
  where dota_account_id = p_dota_account_id;
  if not found then
    return null;
  end if;
  return archive_private.archive_page(v_tracked_account_id, p_period, p_mode, p_result, p_party, p_position, p_hero_id, p_cursor_start_time, p_cursor_match_id, p_limit, p_start_date, p_end_date);
end;
$function$;

revoke all on function archive_private.archive_overview(uuid, text, text, text, text, text, smallint, date, date) from public, anon, authenticated;
revoke all on function archive_private.archive_page(uuid, text, text, text, text, text, smallint, bigint, bigint, integer, date, date) from public, anon, authenticated;
revoke all on function public.get_match_archive_overview(uuid, text, text, text, text, text, smallint, date, date) from public, anon, authenticated;
revoke all on function public.get_match_archive_page(uuid, text, text, text, text, text, smallint, bigint, bigint, integer, date, date) from public, anon, authenticated;
revoke all on function public.get_archive_showcase_overview(bigint, text, text, text, text, text, smallint, date, date) from public, anon, authenticated;
revoke all on function public.get_archive_showcase_page(bigint, text, text, text, text, text, smallint, bigint, bigint, integer, date, date) from public, anon, authenticated;
